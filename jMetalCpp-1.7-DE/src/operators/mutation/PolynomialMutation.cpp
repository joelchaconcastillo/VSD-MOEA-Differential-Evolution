  ...
  for (int var=0; var < solution->getNumberOfVariables(); var++) {
    if (PseudoRandom::randDouble() <= probability) {
     ...
     // delta2 = (yu-y)/(yu-yl)+1e-10;
      ...
      if (y<yl)
        y = yl;
	//cout <<fixed << setprecision(50) << "val "<< val << " deltaq "<< deltaq << " rnd " << rnd<<"  y " << y<< "  delta1 "<<delta1 << "  xy "<<xy <<endl;
    //  if (y< x->getLowerBound(var))
    //    y = x->getLowerBound(var);
      if (y>yu)
        y = yu;
      x->setValue(var, y);
    }
