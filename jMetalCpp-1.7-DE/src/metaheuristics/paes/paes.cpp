...
SolutionSet * paes::execute() {
    ...
    do {
      ...
      if(evaluations % (int)( ((double)maxEvaluations)*0.05) == 0)
	   {
		  cout << evaluations <<endl;
		   if(flag2)
		   {
			archive->printObjectivesToFile(FileNameObjective); /////////////////
			archive->printVariablesToFile(FileNameVariable);
			flag2=false;
		   }
		   else
		  {
			archive->printObjectivesToFile(FileNameObjective, true); //////////////////
			archive->printVariablesToFile(FileNameVariable, true);
		   }
	   }
