...
//    }
  ...
  if( iteration_ % (int)( ((double)maxIterations_)*0.05) == 0)
   {
	  cout << iteration_<<endl;
	   if(flag)
	   {
		leaders_->printObjectivesToFile(FileNameObjective); /////////////////
		leaders_->printVariablesToFile(FileNameVariable);
		flag=false;
	   }
	   else
	  {
	   	leaders_->printObjectivesToFile(FileNameObjective, true); //////////////////
	   	leaders_->printVariablesToFile(FileNameVariable, true);
	   }
   }
