...
*/
  ...
  initPopulation();
		...
		population_->printObjectivesToFile(FileNameObjective); /////////////////
		population_->printVariablesToFile(FileNameVariable);
  ...
  do {
    ...
    for (int i = 0; i < populationSize_; i+=2) {
      ...
      //int n = i ; // or int n = i;
      int type;
      double rnd = PseudoRandom::randDouble(Seed);
...
//      parents[2] = population_->get(n);
      ...
      // Apply SBX crossover
      Solution ** child = (Solution **) (crossover_->execute(parents, maxEvaluations, evaluations_, parents[0]->getNumberOfVariables()));
      //Solution ** child = (Solution **) (crossover_->execute(parents));
      // Apply DE crossover
...
//{ 
      // Apply mutation
      mutation_->execute(child[0]);
      
      // Evaluation
      problem_->evaluate(child[0]);
      ...
      // STEP 2.4. Update z_
      updateReference(child[0]);
      
      // STEP 2.5. Update of solutions
      updateProblem(child[0], n, type);
//	delete child[1];
//}
//else{
// Apply mutation
      mutation_->execute(child[1]);
      
      // Evaluation
      problem_->evaluate(child[1]);
      ...
      // STEP 2.4. Update z_
      updateReference(child[1]);
      
      // STEP 2.5. Update of solutions
      updateProblem(child[1], n, type);
//	delete child[0];
//}
	//child[0] se elimina en la función updateProblem....
	//delete child[1];
	delete [] child;
    } // for

   //if(evaluations_ % (int)( ((double)maxEvaluations)*0.004) == 0 || (evaluations_ < ( ((double)maxEvaluations)*0.06) && ( evaluations_ % (populationSize_*100) )==0   ) )
//	if( !(evaluations_ % ( 2500*populationSize_  )))
   {
...
//getchar();      
	 ...
	 //  else
	 // {
	   	population_->printObjectivesToFile(FileNameObjective, true); //////////////////
	   	population_->printVariablesToFile(FileNameVariable, true);
	 //  }
   }
...
//  else {
    ...
    //os <<  "/home/user_demo/Chacon/Tesis/jMetalCpp-1.7/data/Weight/" << "W" << problem_->getNumberOfObjectives() << "D_" << populationSize_ << ".dat";
    os <<  "/home/user_demo/Chacon/Tesis/jMetalCpp-1.7/data/Weight/" << "Uniform" << problem_->getNumberOfObjectives() << "D-" << populationSize_ << ".dat";
    string dataFileName;
    dataFileName = os.str();
