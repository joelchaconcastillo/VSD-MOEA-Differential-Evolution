...
void OMOPSO::initParams() {
  ...
  FileNameObjective = * (string *) getInputParameter("FileNameObjective");  
  FileNameVariable = * (string *) getInputParameter("FileNameVariable");  
  Seed = *(int *) getInputParameter("Seed");
...
if(iteration% (int)( ((double)maxIterations)*0.05) == 0)
   {
	  cout << iteration <<endl;
	   if(flag2)
	   {
		leaders->printObjectivesToFile(FileNameObjective+"_Leader"); /////////////////
		eArchive->printObjectivesToFile(FileNameObjective); /////////////////
		leaders->printVariablesToFile(FileNameVariable+"_Leader");
		eArchive->printVariablesToFile(FileNameVariable);
		
		flag2=false;
	   }
	   else
	  {
	   	leaders->printObjectivesToFile(FileNameObjective+"_Leader", true); //////////////////
	   	eArchive->printObjectivesToFile(FileNameObjective, true); //////////////////
	   	leaders->printVariablesToFile(FileNameVariable+"_Leader", true);
	   	eArchive->printVariablesToFile(FileNameVariable, true);
	   }
   }
