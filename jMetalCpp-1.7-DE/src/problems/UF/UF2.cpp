...
const double UF2::PI = 3.141592653589793;

UF2::UF2(string solutionType, int numberOfVariables, int numberOfObjectives) {
...
void UF2::evaluate(Solution *solution) {
	...
	for(j = 2; j <= nx; j++) 
	{
		if(j % 2 == 0) 
		{
			yj = x_[j-1]-0.3*x_[0]*(x_[0]*cos(24.0*PI*x_[0]+4.0*j*PI/nx)+2.0)*sin(6.0*PI*x_[0]+j*PI/nx);
			sum2 += yj*yj;
			count2++;
		} 
		else 
		{
			yj = x_[j-1]-0.3*x_[0]*(x_[0]*cos(24.0*PI*x_[0]+4.0*j*PI/nx)+2.0)*cos(6.0*PI*x_[0]+j*PI/nx);
			sum1 += yj*yj;
			count1++;
		}
	}
