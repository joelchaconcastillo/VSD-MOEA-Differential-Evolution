...
const double UF9::PI = 3.141592653589793;

UF9::UF9(string solutionType, int numberOfVariables, int numberOfObjectives) {
...
void UF9::evaluate(Solution *solution) {
	...
	yj = (1.0+E)*(1.0-4.0*(2.0*x_[0]-1.0)*(2.0*x_[0]-1.0));
	if(yj<0.0) yj = 0.0;
	fx_[0] = 0.5*(yj + 2*x_[0])*x_[1]+ 2.0*sum1 / (double)count1;
	fx_[1] = 0.5*(yj - 2*x_[0] + 2.0)*x_[1] + 2.0*sum2 / (double)count2;
	fx_[2] = 1.0 - x_[1]+ 2.0*sum3 / (double)count3;
